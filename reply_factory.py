...
def record_current_answer(answer, current_question_id, session):
    ...
    if current_question_id >= 0 and current_question_id < len(PYTHON_QUESTION_LIST):
...
def generate_final_response(session):
    ...
    for i, question in enumerate(PYTHON_QUESTION_LIST):
        if session["answers"].get(i) == question["answer"]:
            correct_answers += 1
    
    score = (correct_answers / total_questions) * 100
    final_response = f"Your final score is {score:.2f}%."
